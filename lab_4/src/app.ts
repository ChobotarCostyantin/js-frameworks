  ...
  private loadInitialData() {
    ...
    savedBooks.forEach(book =>
      this.library.addItem(
        new Book(book.id, book.title, book.author, book.year, book.available)
      )
    );
  }
  ...
  private attachEventListenersToUsersForm() {
    ...
    if (userForm) {
      userForm.addEventListener('submit', e => {
        ...
        if (
          isValid &&
          Validation.validateUserInput(nameInput.value, emailInput.value)
        ) {
          let newUser = new User(Date.now(), nameInput.value, emailInput.value);
          Array.from(
            document.getElementById('userBooksList').getElementsByTagName('li')
          )
    ...
    Array.from(deleteBookButtons).forEach(el =>
      el.addEventListener('click', e => {
        this.libraryService.removeUser(
          this.storage.getUserById(+el.getAttribute('userid')) as User
        );
        this.initializeApp();
    ...
    if (searchInput) {
      searchInput.addEventListener('input', e => {
        ...
        document.getElementById('bookList').innerHTML =
          this.libraryService.renderBookList(books, this.itemsPerPage);
  ...
  private showPopup(book: Book) {
    ...
    if (form) {
      form.addEventListener('submit', e => {
        e.preventDefault();
        const id = (document.getElementById('user-id') as HTMLInputElement)
          .value;
